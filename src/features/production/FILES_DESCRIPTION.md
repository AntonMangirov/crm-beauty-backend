# 📋 Описание файлов для продакшена

## 🔒 Безопасность

### ✅ `rateLimit.ts` → `src/middleware/security.ts` (УЖЕ РЕАЛИЗОВАНО)

**Назначение:** Защита от брутфорс атак и злоупусений API

- **authRateLimit:** 5 попыток логина за 15 минут
- **publicRateLimit:** 200 запросов к публичному API за 15 минут
- **bookingRateLimit:** 10 записей в час
- **generalRateLimit:** 100 запросов к API за 15 минут

**Статус:** ✅ Уже реализовано в `src/middleware/security.ts`

---

### `emailController.ts` → `src/controllers/emailController.ts`

**Назначение:** Управление email верификацией и сбросом пароля

- **sendEmailVerification():** Отправка письма подтверждения email
- **verifyEmail():** Подтверждение email по токену
- **requestPasswordReset():** Запрос сброса пароля
- **resetPassword():** Сброс пароля по токену

**API эндпоинты:**

- `POST /api/auth/send-verification` - отправка письма
- `GET /api/auth/verify-email/:token` - подтверждение email
- `POST /api/auth/forgot-password` - запрос сброса
- `POST /api/auth/reset-password/:token` - сброс пароля

**Когда использовать:** При необходимости подтверждения email, интеграции с email сервисом

---

### `tokens.ts` → `src/utils/tokens.ts`

**Назначение:** Генерация и управление различными типами токенов

- **generateEmailVerifyToken():** Токен для подтверждения email
- **generatePasswordResetToken():** Токен для сброса пароля
- **generateRefreshToken():** Refresh token для долгосрочной аутентификации
- **createPasswordResetToken():** Токен сброса с истечением через 1 час
- **createRefreshToken():** Refresh token с истечением через 7 дней
- **createAccessToken():** Access token с истечением через 15 минут
- **isTokenExpired():** Проверка истечения токена

**Когда использовать:** При добавлении email верификации, сброса пароля, refresh tokens

---

### `argon2Password.ts` → `src/utils/argon2Password.ts`

**Назначение:** Безопасное хеширование паролей для продакшена

- **hashPassword():** Хеширование пароля (Argon2 для продакшена, bcrypt для разработки)
- **verifyPassword():** Проверка пароля
- **isBcryptHash():** Определение типа хеша
- **migratePasswordHash():** Миграция с bcrypt на Argon2

**Преимущества Argon2:**

- Более безопасный алгоритм
- Защита от атак по времени
- Рекомендован OWASP
- Устойчив к GPU атакам

**Настройки Argon2:**

- memoryCost: 64 MB (2^16)
- timeCost: 3 итерации
- parallelism: 1 поток

**Когда использовать:** Перед деплоем в продакшен, при обновлении системы безопасности

---

## 🔄 Аутентификация

### `refreshTokens.ts` → `src/controllers/refreshController.ts`

**Назначение:** Управление долгосрочной аутентификацией через refresh tokens

- **refreshToken():** Обновление access token с помощью refresh token
- **revokeToken():** Отзыв refresh token (выход из системы)
- **issueRefreshToken():** Выдача refresh token при логине

**API эндпоинты:**

- `POST /api/auth/refresh` - обновление access token
- `POST /api/auth/revoke` - отзыв refresh token

**Безопасность:**

- Access token: 15 минут жизни
- Refresh token: 7 дней жизни
- Автоматическая ротация токенов
- Возможность отзыва токенов

**Когда использовать:** При необходимости долгосрочной аутентификации, переходе на короткоживущие access tokens

---

## 📧 Email (будущие файлы)

### `emailService.ts` → `src/services/emailService.ts`

**Назначение:** Сервис отправки email (SendGrid, AWS SES, Nodemailer)

- Поддержка различных email провайдеров
- Шаблоны писем
- Очереди отправки

### `emailTemplates.ts` → `src/templates/emailTemplates.ts`

**Назначение:** Шаблоны писем для различных событий

- Подтверждение email
- Сброс пароля
- Уведомления

### `emailQueue.ts` → `src/queues/emailQueue.ts`

**Назначение:** Очередь отправки email

- Асинхронная отправка
- Retry логика
- Мониторинг

---

## 🎯 Порядок добавления функций:

### Этап 1: Базовая безопасность

1. **Rate Limiting** - защита от брутфорса
2. **Улучшенная валидация** - проверка данных
3. **Логирование** - отслеживание событий

### Этап 2: Email система

1. **Email верификация** - подтверждение email
2. **Сброс пароля** - восстановление доступа
3. **Email сервис** - интеграция с провайдером

### Этап 3: Продвинутая аутентификация

1. **Refresh tokens** - долгосрочная аутентификация
2. **Argon2 пароли** - безопасное хеширование
3. **Короткоживущие токены** - улучшенная безопасность

### Этап 4: Мониторинг

1. **Логирование** - детальные логи
2. **Метрики** - производительность
3. **Алерты** - уведомления о проблемах

---

## ⚠️ Важные моменты:

1. **Тестируйте пошагово** - добавляйте функции постепенно
2. **Создавайте миграции** - для каждого изменения схемы БД
3. **Обновляйте типы** - TypeScript должен знать о новых полях
4. **Тестируйте API** - проверяйте все эндпоинты
5. **Документируйте** - обновляйте README с новыми функциями
